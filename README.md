...
Raw memos are append-only: corrections are captured as new memos, while AI-generated Markdown and Notion pages are treated as derived documents. Local memo operations, AI draft generation, review state transitions, and approval-gated Notion publishing run through one frontend-neutral application API; the current CLI is its first frontend, and a future desktop frontend can invoke the same API from a global shortcut.
